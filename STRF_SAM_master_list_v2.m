...
%Opening master list
%[~,~,raw]   =   xlsread('c:\work\code_fr\SAM_short_master_list.xlsx');
load('SAM_short_master_list.mat');
s_bin=0.15;
indexn=find(~isnan([raw{:,6}]'));
for n=36:length(indexn)
...
for chi=1:96
    ...
    if ~isempty(sortcode)
        ...
        for n=1:length(s_index)
             temp(n)=length(find(sortcode==s_index(n)));
        end
        intemp=find(temp==max(temp));    
        index=find(sortcode==sortcode(intemp));
        spet=spet(index);
    else
        spet=[];
    end
...
save_strf=['C:\work\SAM_STRF_tdt\' session '\Block-' num2str(block) '\xpz2\'];
...
save_strf=['C:\work\SAM_STRF_tdt\' session '\Block-' num2str(block) '\xpz5\'];
if ~exist(save_strf,'dir')
    mkdir(save_strf)
end
print([save_strf 'STRF_Ch' num2str(chi)],'-djpeg');
