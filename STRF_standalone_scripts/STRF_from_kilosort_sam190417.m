...
path='D:\SpikeSortingPipeline\Tanks\SAM-190417\Block-4\SAM-190417_Block-4_xpz5_ch66.sev';
[TrigA,TrigB]=getting_triggers(path);
%load('D:\SpikeSortingPipeline\Tanks\SAM-190325\SAM-190401-Triggers_block1_STRF.mat')
warning('off','MATLAB:interp1:UsePCHIP')
sprfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50.spr';
%savepath='H:\DataTanks\SAM-180417\Block-1\';
savepath='C:\work\ToSort_Sam\SAM-190417_b4_xpz5AL\STRF\';
s_bin=0.15;
cs=1;
%Getting spet from kilosort result
%%
load('C:\work\ToSort_Sam\SAM-190417_b4_xpz5AL\rez2.mat'); %need to see rez2 probably the same
s_bin=0.15;
cs=1;
spikes=rez.st3;
...
for s=9:10%length(index_clusters) 
 ...
 try   
        ...
        [taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdbint(sprfile,0,s_bin,spet',TrigA,fs,80,30,'dB','MR',100,5,'float');
 catch me
      STRF1A=[];STRF2A=[];No1A=[];Wo1A=[];No1A=[];Wo1A=[]; Xa=[];
    Na=[];
    Ya=[];
 end
 
 try
        [taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdbint(sprfile,0,s_bin,spet',TrigB,fs,80,30,'dB','MR',100,5,'float');
 catch me
...
%STRFDataxpz5_AL(conta) = struct('cluster',cluster_number,'No1',No1,'Wo1',Wo1,'No2',No2,'Wo2',Wo2,'STRF1',STRF1,'STRF2',STRF2,'taxis',taxis,'faxis',faxis,'PP',PP,'SPLN',SPLN);
figure;
taxis = (taxis)*1e3;
faxis = (faxis)*1e3;
%subplot(2,1,1)
...
save_strf='C:\work\ToSort_Sam\SAM-190417_b4_xpz5AL\STRF\';
...
fs=24414.0625;
paramfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50_param.mat';
%load('D:\SpikeSortingPipeline\Tanks\SAM-190325\SAM-190325-Triggers_block1_STRF.mat')
sprfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50.spr';
load('C:\work\ToSort_Sam\SAM-190417_b4_xpz2A1\rez2.mat');
spikes=rez.st3;
%for ks2 cluster is at column 2
index_clusters=unique(spikes(:,2));
%index_clusters=unique(spikes(:,5));
...
for s=1:length(index_clusters) 
 ...
 try
  ...
  STRF1B=[];STRF2B=[];No1B=[];Wo1B=[];No1B=[];Wo1B=[];  Xb=[];
    Nb=[];
    Yb=[];
 end
...
% STRFDataxpz2_ml(conta) = struct('cluster',cluster_number,'No1',No1,'Wo1',Wo1,'No2',No2,'Wo2',Wo2,'STRF1',STRF1,'STRF2',STRF2,'taxis',taxis,'faxis',faxis,'PP',PP,'SPLN',SPLN);
figure;
taxis = (taxis)*1e3;
faxis = (faxis)*1e3;
%subplot(2,1,1)
...
save_strf='C:\work\ToSort_Sam\SAM-190417_b4_xpz2A1\STRF\';
if ~exist(save_strf,'dir')
    mkdir(save_strf)
end
...
%save(save_strf,'STRFDataxpz2_A1','-v7.3');     
...
% %load('D:\SpikeSortingPipeline\Tanks\SAM-190325\SAM-190325-Triggers_block1_STRF.mat')
% sprfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50.spr';
% load('C:\work\ToSort_Sam\SAM-190417_b3_xpz2ML\rez2.mat');
% spikes=rez.st3;
% %for ks2 cluster is at column 2
% index_clusters=unique(spikes(:,2));
% %index_clusters=unique(spikes(:,5));
...
% %colormap jet;set(gca,'YDir','normal'); shading flat;colormap jet;
% 
% save_strf='C:\work\ToSort_Sam\SAM-190417_b3_xpz2ML\STRF\';
% if ~exist(save_strf,'dir')
%     mkdir(save_strf)
% end
% print([save_strf 'STRF_Ch' num2str(cluster_number)],'-djpeg');
