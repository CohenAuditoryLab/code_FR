...
for n=1:1
session     =  'SAM-190318';% raw{indexn(n),4};
block       =  1;%raw{indexn(n),6};
task        =  'STRF';% raw{indexn(n),7};
...
for chi=33:128
    ...
    [Data] = readtank_sdk_new_tanks(['D:\SpikeSortingPipeline\Tanks_192ch\' session],block,chi,'XPZ2');  
...
for chi=1:96
[Data] = readtank_sdk_new_tanks(['D:\SpikeSortingPipeline\Tanks_192ch\' session],block,chi,'XPZ5');  
    TrigTimes=round(Data.Fs*Data.Trig);
    [TrigA,TrigB]=trigfixstrf2(TrigTimes',400,899);
    if chi<=64
