...
%path='D:\SpikeSortingPipeline\Tanks\SAM-1903025\Block-1\SAM-190306_Block-1_xpz5_ch66.sev';
...
savepath='C:\work\ToSort_Sam\SAM-190306_b1_xpz5AL\STRF\';
s_bin=0.15;
cs=1;
%Getting spet from kilosort result
%%
load('C:\work\ToSort_Sam\SAM-190306_b2_xpz5AL\rez2.mat'); %need to see rez2 probably the same
s_bin=0.15;
cs=1;
spikes=rez.st3;
...
save_strf='C:\work\ToSort_Sam\SAM-190306_b2_xpz5AL\STRF\';
...
load('D:\SpikeSortingPipeline\Tanks\SAM-190325\SAM-190325-Triggers_block1_STRF.mat')
sprfile='C:\work\STRF\Moving_ripple\DMR_50HZ\DNR_Cortex_96k5min_4_50.spr';
load('C:\work\ToSort_Sam\SAM-190325_b1_xpz2A1\rez2.mat');
spikes=rez.st3;
%for ks2 cluster is at column 2
index_clusters=unique(spikes(:,2));
%index_clusters=unique(spikes(:,5));
...
for s=1:length(index_clusters) 
 ...
 try   
        ...
        [taxis,faxis,STRF1A,STRF2A,PP,Wo1A,Wo2A,No1A,No2A,SPLN]=rtwstrfdbint(sprfile,0,s_bin,spet'/fs,TrigA/fs,fs,80,30,'dB','MR',100,5,'float');
 catch me
      STRF1A=[];STRF2A=[];No1A=[];Wo1A=[];No1A=[];Wo1A=[];
 end
 
 try
        [taxis,faxis,STRF1B,STRF2B,PP,Wo1B,Wo2B,No1B,No2B,SPLN]=rtwstrfdbint(sprfile,0,s_bin,spet'/fs,TrigB/fs,fs,80,30,'dB','MR',100,5,'float');
 catch me
  STRF1B=[];STRF2B=[];No1B=[];Wo1B=[];No1B=[];Wo1B=[]; 
 end
...
save_strf='C:\work\ToSort_Sam\SAM-190306_b2_xpz2A1\STRF\';
if ~exist(save_strf,'dir')
    mkdir(save_strf)
end
